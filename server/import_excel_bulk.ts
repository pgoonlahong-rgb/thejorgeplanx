...
async function main() {
  ...
  console.log(`Preparing บัญชีโครงการ: ${projRows.length} rows`);
  
  const projValues = projRows.map(row => {
  ...
  for (let i = 0; i < projValues.length; i += 100) {
    const batch = projValues.slice(i, i + 100);
    await db.insert(schema.projects).values(batch);
  }
  console.log("บัญชีโครงการ imported.");

  // 2. โครงการเกินศักยภาพ
  const excSheet = workbook.Sheets["โครงการเกินศักยภาพ"];
  const excRows: any[] = xlsx.utils.sheet_to_json(excSheet);
  console.log(`Preparing โครงการเกินศักยภาพ: ${excRows.length} rows`);

  const excValues = excRows.map(row => {
    const b66 = cleanNum(row[2566]);
    const b67 = cleanNum(row[2567]);
    const b68 = cleanNum(row[2568]);
    const b69 = cleanNum(row[2569]);
  ...
  for (let i = 0; i < excValues.length; i += 100) {
    const batch = excValues.slice(i, i + 100);
    await db.insert(schema.projects).values(batch);
  }
  console.log("โครงการเกินศักยภาพ imported.");

  // 3. บัญชีครุภัณฑ์
  const eqSheet = workbook.Sheets["บัญชีครุภัณฑ์"];
  const eqRows: any[] = xlsx.utils.sheet_to_json(eqSheet);
  console.log(`Preparing บัญชีครุภัณฑ์: ${eqRows.length} rows`);

  const eqValues = eqRows.map(row => {
    const b66 = cleanNum(row[2566]);
    const b67 = cleanNum(row[2567]);
    const b68 = cleanNum(row[2568]);
    const b69 = cleanNum(row[2569]);
  ...
  for (let i = 0; i < eqValues.length; i += 100) {
    const batch = eqValues.slice(i, i + 100);
    await db.insert(schema.equipment).values(batch);
  }
  console.log("บัญชีครุภัณฑ์ imported.");

  console.log("All data imported successfully via bulk insert!");
  process.exit(0);
}
